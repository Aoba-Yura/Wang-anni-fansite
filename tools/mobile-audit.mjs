...
const auditHtml = `<!doctype html><meta charset="utf-8"><style>iframe{display:block;border:0}</style>${frames}<pre id="result"></pre><script>
  ...
  Promise.all(frames.map((frame)=>new Promise((resolve)=>frame.addEventListener('load',()=>setTimeout(resolve,100),{once:true})))).then(()=>{
    const results=frames.map((frame)=>{const doc=frame.contentDocument,win=frame.contentWindow;const overflow=[...doc.querySelectorAll('body *:not([aria-hidden="true"])')].filter((el)=>{const r=el.getBoundingClientRect();return r.right>doc.documentElement.clientWidth+1||r.left<-1}).slice(0,8).map((el)=>String(el.className||el.tagName));const japanese=[...doc.querySelectorAll('.section-heading__title [lang="ja"],.desktop-nav [lang="ja"],.mobile-nav [lang="ja"]')].filter((el)=>{const range=doc.createRange();range.selectNodeContents(el);return range.getClientRects().length>1}).map((el)=>el.textContent.trim()).slice(0,8);const profile=doc.querySelector('.profile-layout');const profileOrder=profile?[...profile.querySelectorAll('.profile-photo,.profile-overline,.profile-content h2,.source-chip,.fact-table')].map((el)=>Math.round(el.getBoundingClientRect().top)):null;const canvas=doc.querySelector('#breakout-game');return{page:frame.dataset.page,viewport:frame.dataset.viewport,scrollWidth:doc.documentElement.scrollWidth,clientWidth:doc.documentElement.clientWidth,overflow,japanese,profileOrder,canvasWidth:canvas?Math.round(canvas.getBoundingClientRect().width):null}});document.querySelector('#result').textContent=JSON.stringify(results)
  });
<\/script>`;
...
try {
  ...
  const failures = results.filter((item) => item.scrollWidth > item.clientWidth || item.overflow.length || item.japanese.length || (item.profileOrder && item.profileOrder.some((top,index,values)=>index>0&&top<values[index-1])) || (item.canvasWidth && item.canvasWidth > Number(item.viewport.split("x")[0])));
