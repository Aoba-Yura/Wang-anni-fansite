...
try {
  ...
  const failures = results.filter((item) => item.scrollWidth > item.clientWidth || item.overflow.length || item.japanese.length || (item.profileOrder && item.profileOrder.some((top,index,values)=>index>0&&top<values[index-1])) || (item.canvasWidth && item.canvasWidth > Number(item.viewport.split("x")[0])) || (item.diary && (item.diary.heroHeight > 190 || !item.diary.aligned || !item.diary.labelAbove || item.diary.buttonHeights.some((height)=>height<40||height>48) || item.diary.buttonLines.some((lines)=>lines!==1) || item.diary.firstVisible<120 || item.diary.navHeight>92 || item.diary.lastBottomAtMax>item.diary.navTop-24)));
