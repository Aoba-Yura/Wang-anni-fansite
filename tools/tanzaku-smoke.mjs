...
try {
  ...
  if (result.desktop.variationCount !== result.desktop.poems || result.mobile.variationCount !== result.mobile.poems) failures.push("tanzaku material variations are not unique");
  ...
  if (![result.desktop, result.mobile].every(({ phraseRects }) => phraseRects.length === 3 && Math.max(...phraseRects.map((rect) => rect.x)) - Math.min(...phraseRects.map((rect) => rect.x)) <= 1 && phraseRects.every((rect, index) => !index || rect.top > phraseRects[index - 1].bottom))) failures.push("phrases are not separated on one vertical axis");
  const mobilePaperRatio = result.mobile.paper[0] / result.mobile.viewport[0];
  if (String(result.desktop.paper) !== "143,560" || mobilePaperRatio < .33 || mobilePaperRatio > .36 || Math.abs(result.mobile.paper[1] / result.mobile.paper[0] - 3.78) > .04) failures.push("tanzaku proportions changed");
  ...
  if (result.mobile.layout.paperBottomAtMax > result.mobile.layout.navTop - 16) failures.push("bottom navigation overlaps the tanzaku at maximum scroll");
